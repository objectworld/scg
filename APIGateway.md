...
- [1. API 게이트웨이 기능](#1-api---------)
  
  * [1.1 배경](#11---)
  * [1.2. **API 게이트웨이의 주요 기능**](#12API게이트웨이의주요기능)
    + [1.2.1. **인증/인가에 관련된 기능**](#121------------------)
    + [1.2.2. **API 토큰 발급**](#122---api--------)
    + [1.2.3. **엔드포인트별 API 호출 인증**](#123----------api--------)
    + [1.2.4. **엔드포인트별 API 요청 인가**](#124----------api--------)
    + [1.2.5. **API 라우팅**](#125---api------)
      - [1.2.5.1. **백엔드 API 서버로의 로드 밸런싱**](#1251-------api--------------)
      - [1.2.5.2. **서비스 및 클라이언트 별 엔드포인트 제공**](#1252---------------------------)
      - [1.2.5.3. **메세지 또는 헤더기반 라우팅**](#1253--------------------)
    + [1.2.6. **공통 로직 처리**](#126-------------)
    + [1.2.7. **메디에이션 기능 (Mediation)**](#127-------------mediation---)
      - [**1.2.7.1. 메세지 포맷 변환 (Message format transformation)**](#--1271------------message-format-transformation---)
      - [**1.2.7.2. 프로토콜 변환**](#--1272----------)
    + [1.2.8. **메세지 호출 패턴 변환 (Message Exchange Pattern : MEP)**](#128-----------------message-exchange-pattern---mep---)
    + [1.2.9. **어그레게이션 (aggregation)**](#129-----------aggregation---)
    + [**1.2.10. 로깅 및 미터링**](#--1210-----------)
      - [**1.2.10.1 API 호출 로깅**](#--12101-api--------)
      - [**1.2.10.2. API 미터링 & 차징 (Metering & Charing)**](#--12102-api-----------metering---charing---)
      - [**1.2.10.3. QoS 조정 (Quality of service)**](#--12103-qos-----quality-of-service---)
...
클레임 기반의 토큰이 아닌 경우, 이러한 클레임 정보를 서버에 저장해놓게 되는데, 클라이언트로는 unique한 string만을 리턴해주는 경우이다.
 ...
 <center><img src="/assets/21624D3C54F323CB23.png"><p><그림. 서버에 토큰을 저장하는 경우></p></center>
...
#### 1.2.5.1. **백엔드 API 서버로의 로드 밸런싱**
가장 기본적인 기능으로는 로드밸런서 기능이다. API 게이트 웨이 뒷단에 다수의 API 서버가 있다고 할때, 여러개의 API 서버로 부하를 분산하는 기능이 필요하다.
 <center><img src="/assets/2776453C54F323CC0D.png"><p><그림. API 게이트 웨이를 통한 API 서버로의 로드 밸런싱></p></center>
...
#### 1.2.5.2. **서비스 및 클라이언트 별 엔드포인트 제공**
...
<center><img src="/assets/235AD43C54F323CC2B.png"><p><그림 API 게이트웨이를 이용하여 공통 로직을 API 서버에서 API 게이트웨이로 이전한 구조></p></center>

### 1.2.7. **메디에이션 기능 (Mediation)**
메디에이션이란, 한글로 “중재”또는 “조정” 이라는 의미를 갖는데, API서버에서 제공되는 API가 클라이언트가 원하는 API 형태와 다를때, API 게이트웨이가 이를 변경해주는 기능을 이야기 한다. 구체적인 기능을 보자

#### **1.2.7.1. 메세지 포맷 변환 (Message format transformation)**
...
<center><img src="/assets/225B3D3C54F323CB2B.png"><p><그림. API 게이트 웨이를 통한 프로토콜 변환></p></center>
...
### 1.2.8. **메세지 호출 패턴 변환 (Message Exchange Pattern : MEP)**
...
<center><img src="/assets/23574C3C54F323CB31.png"><p><그림. Apigee.com의 API 모니터링></p></center>

#### **1.2.10.2. API 미터링 & 차징 (Metering & Charing)**	
...
#### **1.2.10.3. QoS 조정 (Quality of service)**
...
<center><img src="/assets/233A833B54F727471C.png"><p><그림. 외부 및 내부 게이트웨이를 분리한 패턴></p></center>
...
<center><img src="/assets/image-20220627173458657.png"><p></p></center>
...
<center><img src="/assets/ICISTR.png"><p></p></center>

## 6.1.  API GW 기능

<center><img src="/assets/api2.png"><p></p></center>
